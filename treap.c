...
struct treap_node *insert(struct treap_node *root, int key) {
  if (!root) return _treap_node_new(key);

  if (root->key > key) {
    root->left = insert(root->left, key);
    if (root->priority > root->left->priority) root = _rotate_right(root);
  } else if (root->key < key) {
    root->right = insert(root->right, key);
    if (root->priority > root->right->priority) root = _rotate_left(root);
