...
static int _depth(struct avl_node *root) {
  int d1, d2;
 ...
 * "nil"s
 */
void rb_dump(struct avl_node *root) {
  ...
  enqueue(queue, root);
  while (queue->size) {
    root = (struct avl_node *)dequeue(queue);
    ...
    if (!root) {
      ...
      printf("%d%d", root->val, root->height ? 'b' : 'r');
...
int avl_check_height(struct avl_node *root) {
 ...
 */
int avl_valid(struct avl_node *root) { return avl_check_height >= 0; }
...
int main() {
  ...
  for (int i = 0; i < sizeof test_data / sizeof test_data[0]; i++) {
    root = avl_insert(root, test_data[i]);
    assert(avl_valid(root));
    assert(avl_find(root, test_data[i]));
  }

  // test deleting
  for (int i = 0; i < sizeof test_data / sizeof test_data[0]; i++) {
    root = avl_remove(root, test_data[i]);
    assert(avl_valid(root));
    assert(!avl_find(root, test_data[i]));
  }
  printf("success!\n");
