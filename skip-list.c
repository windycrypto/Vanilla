...
void insert(struct skip_list *list, int key)
{
	struct skip_list_node
		*n = &list->head, // a cursor used to traverse the list
		*prev[SKIP_LIST_MAX_LEVEL] = {
			NULL
		}; // stores n's previous nodes
	int lvl;

	// list should not be NULL if the user create the skip list using `skip_list_new`
	if (!list)
		abort();

	lvl = 0;
	for (; lvl < SKIP_LIST_MAX_LEVEL; lvl++)
		prev[lvl] = &list->head;
...
void delete (struct skip_list *list, int key)
{
	...
	// till now, prev's all levels' next pointers point to the deleting node's next node
	// and target point to the deleting node(if we found it), since no pointers point
	// to it now, we can just free it
	if (target)
		free(target);
}
